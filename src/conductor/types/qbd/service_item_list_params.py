...
__all__ = ["ServiceItemListParams"]


class ServiceItemListParams(TypedDict, total=False):
    ...
    """Filter for service items of this class or classes.

    Specify a single class ID or multiple using a comma-separated list (e.g.,
    `classIds=1,2,3`). A class is a way end-users can categorize service items in
    QuickBooks.
    """
    ...
    """Filter for specific service items by their full-name(s).
    ...
    """Filter for service items whose `name` contains this substring (case
    insensitive).
    ...
    """
    Filter for service items whose `name` ends with this substring (case
    ...
    """
    Filter for service items whose `name` starts with this substring (case
    insensitive). If you use this parameter, you cannot use `nameContains` or
    `nameEndsWith`.
    """
